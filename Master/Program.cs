...
namespace TMP2
{
    class main
    {
        ...
        {
            //Задаем параметры для уравнения теплопроводности
            //double X = 2.0;
            int Count = 0, Count_T = 1, Count_Parts_T;
            string[] More_Phi, Function_U, Function_Part_U , Function_Part_U_First;
            ...
            Function_Part_U_First = new string[Count_Parts_T];
            ...
            string Var_P_x_new = "P(x) новый";
            if (!dataset.Any(v => v.Name == Var_P_x_new))
                dataset.Add<double[]>(Var_P_x_new);

            string Var_F_x = "F(x)";
            if (!dataset.Any(v => v.Name == Var_F_x))
                dataset.Add<double[]>(Var_F_x);
            ...
            double[][] Result_First = new double[My_Class.Count_Parts_For_T + 1][];
            for (int i = 0; i < My_Class.Count_Parts_For_T + 1; i++)
                Result_First[i] = new double[My_Class.Count_Parts_For_X];

            Result_First = My_Class.First_Tridiagonal_Matrix_Algorithm(My_Class.Lenght_X, My_Class.Lenght_T);
            for (int i = 0; i < Count_Parts_T; i++)
            {
                Function_Part_U_First[i] = "u(x, t = " + (i + 1) * Step_On_T + ")" +" Прямая задача";
                if (!dataset.Any(v => v.Name == Function_Part_U_First[i]))
                    dataset.Add<double[]>(Function_Part_U_First[i]);
            }

            double[] p_x_new = new double[My_Class.Count_Parts_For_X];
            for (int i = 0; i < My_Class.Count_Parts_For_X; i++)
            {
                p_x_new[i] = 2 + Math.PI * Math.PI / (1 - Math.Pow(Math.E, (-Math.PI * Math.PI))) * Math.Cos(Math.PI * (i * My_Class.Lenght_OF_One_Step_On_X));
            }

            double[] f_x = new double[My_Class.Count_Parts_For_X];
            for (int i = 0; i < My_Class.Count_Parts_For_X; i++)
            {
                f_x[i] = My_Class.F_x(i * My_Class.Lenght_OF_One_Step_On_X);
            }
            ...
            dataset.PutData<double[]>(Var_P_x_new, p_x_new);
            dataset.PutData<double[]>(Var_F_x, f_x);
            ...
            for (int j = 0; j < Count_Parts_T; j++)
            {
                for (int i = 0; i < (My_Class.Count_Parts_For_T / Count_Parts_T) * (j + 1) + 1; i++)
                {
                    dataset.PutData<double[]>(Function_Part_U_First[j], Result_First[i]);
        ...
        {
            double p_x = 0.0;
            p_x = 2 + Math.PI * Math.PI / (1 - Math.Pow(Math.E, (-Math.PI * Math.PI)));
